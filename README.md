...
* 방법: 매 트랙마다 각 선수에 대응하는 평균이 현재 Elo 점수이고 스케일이 400인 로지스틱 분포에서 숫자를 하나씩 추출하여 그 순서대로 순위를 부여, 그에 맞춰 Elo 점수와 경기 점수를 업데이트. 이를 경기 종료 조건이 될 때까지 반복.
...
승부예측이 가장 어려운 경기 중 하나. 올해 경기 중 시뮬레이션 1위 확률이 20%를 넘는 사람이 한 명도 나오지 않은 경기는 이 경기와 [이번 시즌 32강 패자부활전](./rounds/2019_2_2.md) 뿐. 승자전 4명과 패자전 4명의 진출 확률이 약간 갈려 있긴 하지만 참가자들의 Elo 점수가 이렇게 비슷한 상황에서 확실하게 말할 수 있는 것은 없다.
...
* 2019년 승부예측 성공률: 
  ...
  * 해당 인원보다 1명 더 선택할 수 있을 때 성공률: 
    * 상위 라운드 진출자 70% (49/70)
    * 1위 65% (13/20)    
  * 예선이나 연방, 비방용 대회 성적을 전혀 고려하지 않았다는 점을 생각할 때 꽤 괜찮은 것으로 보임.
